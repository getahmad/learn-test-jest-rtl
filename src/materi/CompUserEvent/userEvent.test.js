...
test("should render onchange event ketika typing", () => {
  render(<App />);
  const input = screen.getByPlaceholderText("input value");
  userEvent.type(input, "Ahmad Danang");
  expect(screen.getByText(/anda mengetik Ahmad Danang/)).toBeInTheDocument();
});

test("should click in comp user event", () => {
